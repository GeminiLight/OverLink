            ...
            <div className="flex justify-between items-start mb-6">
                <div className="space-y-1">
                    ...
                    {project.last_sync_at && (
                        <div className="flex items-center gap-2 mt-2 opacity-40 bg-slate-100 dark:bg-white/5 px-3 py-1 rounded-full w-fit">
                            <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                            <span className="text-[10px] font-black uppercase tracking-widest">
                                {(t.misc as any).lastUpdated}: {new Date(project.last_sync_at).toLocaleString(lang, { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' })}
                            </span>
                        </div>
                    )}
